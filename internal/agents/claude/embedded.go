...
func (c *Client) MCPServers(ctx context.Context) ([]agents.MCPServer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("claude: mcpServer/list: %w", err)
	}
	...
	if jsonErr := json.Unmarshal(result.Result, &payload); jsonErr != nil {
		return nil, fmt.Errorf("claude: decode mcpServer/list result: %w", jsonErr)
	}
...
func (c *Client) MCPCall(ctx context.Context, params agents.MCPCallParams) (agents.MCPCallResult, error) {
	if c == nil {
		return agents.MCPCallResult{}, errors.New("claude: nil client")
	}
	params.Server = strings.TrimSpace(params.Server)
	params.Tool = strings.TrimSpace(params.Tool)
	if params.Server == "" {
		return agents.MCPCallResult{}, errors.New("claude: server is required")
	}
	if params.Tool == "" {
		return agents.MCPCallResult{}, errors.New("claude: tool is required")
	}
	if ctx == nil {
		ctx = context.Background()
	}
	...
	if err != nil {
		return agents.MCPCallResult{}, fmt.Errorf("claude: mcpServer/call: %w", err)
	}

	var payload agents.MCPCallResult
	if jsonErr := json.Unmarshal(result.Result, &payload); jsonErr != nil {
		return agents.MCPCallResult{}, fmt.Errorf("claude: decode mcpServer/call result: %w", jsonErr)
	}
	return payload, nil
}
...
func (c *Client) MCPOAuth(ctx context.Context, server string) (agents.MCPOAuthResult, error) {
	if c == nil {
		return agents.MCPOAuthResult{}, errors.New("claude: nil client")
	}
	server = strings.TrimSpace(server)
	if server == "" {
		return agents.MCPOAuthResult{}, errors.New("claude: server is required")
	}
	if ctx == nil {
		ctx = context.Background()
	}
	...
	if err != nil {
		return agents.MCPOAuthResult{}, fmt.Errorf("claude: mcpServer/oauth/login: %w", err)
	}

	var payload agents.MCPOAuthResult
	if jsonErr := json.Unmarshal(result.Result, &payload); jsonErr != nil {
		return agents.MCPOAuthResult{}, fmt.Errorf("claude: decode mcpServer/oauth/login result: %w", jsonErr)
	}
	return payload, nil
}
